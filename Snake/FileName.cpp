...
#pragma comment(lib, "winmm.lib") // 链接多媒体库以支持音频功能
...
// --- 配置管理 ---
struct GameConfig {
    int sfxVolume = 50;     // 音效大小
    int musicVolume = 40;   // 音乐大小
    int masterVolume = 80;  // 整体音量
...
private:
    ...
    bool dirLocked = false; // 转向锁，防止单帧180度自杀
...
public:
    SnakeGame() : state(GameState::MAIN_MENU), menuCursor(0), currentMap(1) {
...
private:
    ...
    void drawUIFrame(const wstring& title) {
